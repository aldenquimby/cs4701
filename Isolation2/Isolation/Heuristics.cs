...
namespace Isolation
{
    ...
    public class HeuristicCache
    {
        ...
        #endregion
        private readonly MemoryCache _cache = new MemoryCache("isolation", new NameValueCollection { { "cacheMemoryLimitMegabytes", "1" }, { "physicalMemoryLimitPercentage", "5" } });
        ...
        public int Evaluate(Board board, HeuristicBase heuristic)
        {
            ...
            if (!_cache.Contains(boardString))
            {
                _cache.Add(boardString, new Dictionary<string, int>(), new CacheItemPolicy());
            }

            var innerCache = (Dictionary<string, int>)_cache[boardString];

            if (!innerCache.ContainsKey(heuristic.Name))
            {
                innerCache[heuristic.Name] = heuristic.Evaluate(board);
            }

            return innerCache[heuristic.Name];
        }
        ...
        public void LoadCache(IList<HeuristicDto> dtos)
        {
            ...
            {
                if (!_cache.Contains(dto.Board))
                {
                    _cache[dto.Board] = new Dictionary<string, int>();
                }

                var innerCache = (Dictionary<string, int>)_cache.Get(dto.Board);
                innerCache[dto.Heuristic] = dto.Score;
            }
        }

        public IList<HeuristicDto> DumpCache()
        {
            return new List<HeuristicDto>();
        }
    }
}
