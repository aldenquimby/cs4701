...
namespace Isolation
{
    public class AlphaBeta
    {
        ...
        private BestMoveResult BestMoveInternal(Board board, int depth, int alpha, int beta, CancellationToken cancelToken)
        {
            ...
            var children = validMoves.Select(x => new {move = x, newBoard = board.Copy().Move(x)});

            // sort move list only if we're not near the bottom of the tree, because it's expensive
            if (isMaxTurn)
            {
                children = children.OrderByDescending(x => _evaluator.Evaluate(x.newBoard, _config.Heuristic));
            }
            else
            {
                children = children.OrderBy(x => _evaluator.Evaluate(x.newBoard, _config.Heuristic));
            }

            foreach (var moveAndBoard in children)
            {
                _nodesGeneratedByDepth[depth] = _nodesGeneratedByDepth[depth] + 1;

                BestMoveResult childResult;

                // check quiessence search
                if (IsInterestingMove(board, moveAndBoard.newBoard))
                {
                    // extend search depth because this move looks interesting
                    _numNodesQuiessenceSearched++;
                    childResult = BestMoveInternal(moveAndBoard.newBoard, depth, alpha, beta, cancelToken);
                }
                else
                {
                    // normal evaluation
                    childResult = BestMoveInternal(moveAndBoard.newBoard, depth - 1, alpha, beta, cancelToken);
                }
